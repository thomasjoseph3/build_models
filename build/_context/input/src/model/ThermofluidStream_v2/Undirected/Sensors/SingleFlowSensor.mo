...
model SingleFlowSensor "Flow sensor"
  ...
  import Quantities=ThermofluidStream_v2.Sensors.Internal.Types.MassFlowQuantities;
  ...
  final parameter String quantityString=
    if quantity == ThermofluidStream_v2.Sensors.Internal.Types.MassFlowQuantities.m_flow_kgps then "m_flow in kg/s"
    elseif quantity == ThermofluidStream_v2.Sensors.Internal.Types.MassFlowQuantities.m_flow_gps then "m_flow in g/s"
    elseif quantity == ThermofluidStream_v2.Sensors.Internal.Types.MassFlowQuantities.V_flow_m3ps then "V_flow in m3/s"
    elseif quantity == ThermofluidStream_v2.Sensors.Internal.Types.MassFlowQuantities.V_flow_lpMin then "V_flow in l/min"
    elseif quantity == ThermofluidStream_v2.Sensors.Internal.Types.MassFlowQuantities.H_flow_Jps then "H_flow in J/s"
    elseif quantity == ThermofluidStream_v2.Sensors.Internal.Types.MassFlowQuantities.S_flow_JpKs then "S_flow in J/(K.s)"
    elseif quantity == ThermofluidStream_v2.Sensors.Internal.Types.MassFlowQuantities.Cp_flow_JpKs then "Cp_flow in J/(K.s)"
    else "error";
  ...
  parameter InitMode init=InitMode.steadyState "Initialization mode for sensor output"
    annotation(Dialog(group="Output", enable=outputValue and filter_output));
  parameter Real value_0(unit=ThermofluidStream_v2.Sensors.Internal.getFlowUnit(quantity)) = 0 "Start value of sensor output"
    annotation(Dialog(group="Output", enable=outputValue and filter_output and init==InitMode.state));

  Modelica.Blocks.Interfaces.RealOutput value_out(unit=ThermofluidStream_v2.Sensors.Internal.getFlowUnit(quantity)) = value if outputValue "Sensor output connector"
    annotation (Placement(transformation(extent={{70,50},{90,70}}),
        iconTransformation(extent={{70,50},{90,70}})));

  output Real value(unit=ThermofluidStream_v2.Sensors.Internal.getFlowUnit(quantity));

protected
  Real direct_value(unit=ThermofluidStream_v2.Sensors.Internal.getFlowUnit(quantity));

  function getQuantity = ThermofluidStream_v2.Sensors.Internal.getFlowQuantity (
