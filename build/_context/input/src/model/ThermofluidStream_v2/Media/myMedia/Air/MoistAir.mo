...
package MoistAir "Air: Moist air model (190 ... 647 K)"
  ...
  import ThermofluidStream_v2.Media.myMedia.IdealGases.Common.Functions;
...
Specific enthalpy of liquid water is computed from temperature using a polynomial approach. Kept for compatibility reasons, better use <a href=\"modelica://Modelica.Media.Air.MoistAir.enthalpyOfWater\">enthalpyOfWater</a> instead.
  ...
  algorithm
    h := ThermofluidStream_v2.Media.myMedia.IdealGases.Common.Functions.h_Tlow(
        data=steam,
        T=T,
        refChoice=ReferenceEnthalpy.UserDefined,
        h_off=46479.819 + 2501014.5)*X[Water] +
      ThermofluidStream_v2.Media.myMedia.IdealGases.Common.Functions.h_Tlow(
        data=dryair,
        T=T,
        refChoice=ReferenceEnthalpy.UserDefined,
...
Specific enthalpy of moist air is computed from pressure, temperature and composition with X[1] as the total water mass fraction. The fog region is included for both, ice and liquid fog.
  ...
  algorithm
    ...
    h_der := X_steam*
      ThermofluidStream_v2.Media.myMedia.IdealGases.Common.Functions.h_Tlow_der(
        data=steam,
        T=T,
        refChoice=ReferenceEnthalpy.UserDefined,
        h_off=46479.819 + 2501014.5,
...
The specific heat capacity at constant pressure <strong>cp</strong> is computed from temperature and composition for a mixture of steam (X[1]) and dry air. All water is assumed to be in the vapor state.
  ...
  algorithm
    cv := ThermofluidStream_v2.Media.myMedia.IdealGases.Common.Functions.cp_Tlow(
      dryair, state.T)*(1 - state.X[Water]) +
      ThermofluidStream_v2.Media.myMedia.IdealGases.Common.Functions.cp_Tlow(steam,
...
<p>2019-05-14 Stefan Wischhusen: Corrected derivatives.</p>
  ...
  algorithm
    MM := Modelica.Constants.R/
      ThermofluidStream_v2.Media.myMedia.Air.MoistAir.gasConstant(state);
    annotation (Documentation(revisions="<html>
<p>2012-01-12 Stefan Wischhusen: Initial Release.</p>
</html>"));
  end molarMass;
...
<p>2012-01-12 Stefan Wischhusen: Initial Release.</p>
  ...
  algorithm
    s := ThermofluidStream_v2.Media.myMedia.IdealGases.Common.Functions.s0_Tlow(
      dryair, T)*(1 - X[Water]) +
      ThermofluidStream_v2.Media.myMedia.IdealGases.Common.Functions.s0_Tlow(steam,
      T)*X[Water] - Modelica.Constants.R*(Utilities.smoothMax(
        X[Water]/MMX[Water],
        0.0,
        1e-9)*Modelica.Math.log(max(Y[Water], Modelica.Constants.eps)*p/
      reference_p) + Utilities.smoothMax(
        (1 - X[Water])/MMX[Air],
        0.0,
        1e-9)*Modelica.Math.log(max(Y[Air], Modelica.Constants.eps)*p/
      reference_p));
    annotation (
...
<p>2019-09-10 Stefan Wischhusen: Corrected pressure influence (p &lt; p_ref).</p>
  ...
  algorithm
    ...
    ds := IdealGases.Common.Functions.s0_Tlow_der(
        ...
        T,
        dT)*X[Water] +
      ThermofluidStream_v2.Media.myMedia.IdealGases.Common.Functions.s0_Tlow(
      dryair, T)*dX[Air] +
      ThermofluidStream_v2.Media.myMedia.IdealGases.Common.Functions.s0_Tlow(steam,
      T)*dX[Water] - Modelica.Constants.R*(1/MMX[Water]*(
      Utilities.smoothMax_der(
        ...
        dX[Water],
        0.0,
        0.0)*(Modelica.Math.log(max(Y[Water], Modelica.Constants.eps)*p/
      reference_p) + MM/MMX[Air]) + dp/p*Utilities.smoothMax(
        ...
        dX[Air],
        0.0,
        0.0)*(Modelica.Math.log(max(Y[Air], Modelica.Constants.eps)*p/
...
<p>2019-09-10 Stefan Wischhusen: Corrected pressure influence (p &lt; p_ref).</p>
  ...
  redeclare function extends isentropicEnthalpy
    ...
    extends Modelica.Icons.Function;
  algorithm
    h_is := ThermofluidStream_v2.Media.myMedia.Air.MoistAir.h_pTX(
        p_downstream,
        ThermofluidStream_v2.Media.myMedia.Air.MoistAir.T_psX(
          p_downstream,
          ThermofluidStream_v2.Media.myMedia.Air.MoistAir.specificEntropy(refState),
          refState.X),
        refState.X);
