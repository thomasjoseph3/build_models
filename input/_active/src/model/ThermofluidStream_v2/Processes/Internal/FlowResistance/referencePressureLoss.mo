...
function referencePressureLoss "Pressure loss function based on reference values"
  ...
  input SI.Density rho_ref "Reference density"
    annotation(Dialog(enable=true));
  input ThermofluidStream_v2.Processes.Internal.ReferencePressureDropFunction dp_function=ThermofluidStream_v2.Processes.Internal.ReferencePressureDropFunction.linear "Pressure loss function"
    annotation (Dialog(enable=true));
  input Real m(unit = "1") = 1.5 "Exponent for pressure loss function"
    annotation(Dialog(enable=(dp_function == ThermofluidStream_v2.Processes.Internal.ReferencePressureDropFunction.customExponent)));

algorithm
  if dp_function == ThermofluidStream_v2.Processes.Internal.ReferencePressureDropFunction.linear then
    pressureLoss := rho_ref/rho*dp_ref*m_flow/m_flow_ref;
  elseif dp_function == ThermofluidStream_v2.Processes.Internal.ReferencePressureDropFunction.quadratic then
    pressureLoss := rho_ref/rho*dp_ref*Modelica.Fluid.Utilities.regSquare(m_flow/m_flow_ref);
  elseif dp_function == ThermofluidStream_v2.Processes.Internal.ReferencePressureDropFunction.customExponent then
