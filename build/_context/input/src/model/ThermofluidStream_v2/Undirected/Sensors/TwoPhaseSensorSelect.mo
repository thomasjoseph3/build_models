...
model TwoPhaseSensorSelect "Selectable sensor for two phase medium"
  ...
  import Quantities=ThermofluidStream_v2.Sensors.Internal.Types.TwoPhaseQuantities;
...
<p>Replaceable medium package for the sensor. Medium must be a TwoPase Medium.</p>
  ...
  final parameter String quantityString=
    if quantity == ThermofluidStream_v2.Sensors.Internal.Types.TwoPhaseQuantities.x_kgpkg then "x in kg/kg"
    elseif quantity == ThermofluidStream_v2.Sensors.Internal.Types.TwoPhaseQuantities.T_sat_K then "T_sat in K"
    elseif quantity == ThermofluidStream_v2.Sensors.Internal.Types.TwoPhaseQuantities.T_sat_C then "T_sat in °C"
    elseif quantity == ThermofluidStream_v2.Sensors.Internal.Types.TwoPhaseQuantities.p_sat_Pa then "p_sat in Pa"
    elseif quantity == ThermofluidStream_v2.Sensors.Internal.Types.TwoPhaseQuantities.p_sat_bar then "p_sat in bar"
    elseif quantity == ThermofluidStream_v2.Sensors.Internal.Types.TwoPhaseQuantities.T_oversat_K then "T - T_sat in K"
    elseif quantity == ThermofluidStream_v2.Sensors.Internal.Types.TwoPhaseQuantities.p_oversat_Pa then "p - p_sat in Pa"
    elseif quantity == ThermofluidStream_v2.Sensors.Internal.Types.TwoPhaseQuantities.p_oversat_bar then "p - p_sat in bar"
    else "error";
  ...
  parameter InitMode init=InitMode.steadyState "Initialization mode for sensor output"
    annotation(Dialog(group="Output", enable=outputValue and filter_output));
  parameter Real value_0(unit=ThermofluidStream_v2.Sensors.Internal.getTwoPhaseUnit(quantity)) = 0 "Start value of sensor output"
    annotation(Dialog(group="Output", enable=outputValue and filter_output and init==InitMode.state));

  Modelica.Blocks.Interfaces.RealOutput value_out(unit=ThermofluidStream_v2.Sensors.Internal.getTwoPhaseUnit(quantity)) = value if outputValue "Sensor output connector"
    annotation (Placement(
        transformation(extent={{70,50},{90,70}}),
          iconTransformation(extent={{70,50},{90,70}})));

  Real value(unit=ThermofluidStream_v2.Sensors.Internal.getTwoPhaseUnit(quantity));

protected
  Real direct_value(unit=ThermofluidStream_v2.Sensors.Internal.getTwoPhaseUnit(quantity));

  function getQuantity = ThermofluidStream_v2.Sensors.Internal.getTwoPhaseQuantity (
