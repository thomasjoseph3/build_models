...
model Test_EdgedBend
  ...
  ThermofluidStream_v2.Processes.Pipes.EdgedBend edgedBend(
    redeclare package Medium = Medium,
...
equation
  connect(source.outlet, edgedBend.inlet)
    annotation (Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(sink.inlet, edgedBend.outlet) annotation (Line(
      points={{-5.55112e-16,20},{-5.55112e-16,16},{0,16},{0,10}},
      color={28,108,200},
      thickness=0.5));
  connect(pressure_sine.y, source.p0_var) annotation (Line(points={{-59,6},{-32,6}}, color={0,0,127}));
