within ThermofluidStream_v2.Processes.Internal.FlowResistance;
function laminarTurbulentPressureLoss "Pressure loss function (Cheng 2008) for laminar, transient and turbulent flow regimes"
  ...
  input SI.Length ks_input(min=1e-7) = 1e-7 "Surface roughness"
    annotation(Dialog(enable=(material == ThermofluidStream_v2.Processes.Internal.Material.other)));
  input ThermofluidStream_v2.Processes.Internal.Material material=ThermofluidStream_v2.Processes.Internal.Material.other "Material of pipe"
...
algorithm
  if material == ThermofluidStream_v2.Processes.Internal.Material.concrete then
    ks :=5e-3;
  elseif material == ThermofluidStream_v2.Processes.Internal.Material.wood then
    ks :=0.5e-3;
  elseif material == ThermofluidStream_v2.Processes.Internal.Material.castIron then
    ks :=0.25e-3;
  elseif material == ThermofluidStream_v2.Processes.Internal.Material.galvanizedIron then
    ks :=0.15e-3;
  elseif material == ThermofluidStream_v2.Processes.Internal.Material.steel then
    ks :=0.059e-3;
  elseif material == ThermofluidStream_v2.Processes.Internal.Material.drawnPipe then
    ks :=0.0015e-3;
  else
    ks :=ks_input;
  end if;
...
<p><img src=\"modelica://Thermofluidstream/Resources/Doku/ThermofluidStream_v2.Processes.Internal.FlowResistance.laminarTurbulentPressureLoss.PNG\"/></p>
