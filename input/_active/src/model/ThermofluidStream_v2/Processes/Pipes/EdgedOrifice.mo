...
model EdgedOrifice "Pressure drop due to sharp edged orifice using Modelica.Fluid.Dissipation.PressureLoss.Orifice"
  ...
  extends ThermofluidStream_v2.Interfaces.SISOFlow(final L = if computeL then l_0/(d_0^2*pi/4) else L_value, final clip_p_out = true);
  ...
  parameter SI.Pressure dp_0 = 0 "Initial value for dp" annotation (
    Dialog(tab = "Initialization", group = "dp", enable = (initdp == ThermofluidStream_v2.Utilities.Types.InitializationMethods.state)));
