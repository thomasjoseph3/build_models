within ThermofluidStream_v2.HeatExchangers;
model ShellAndTubeNTU "Shell and tube heat exchanger (1 shell pass, 2n tube passes) using the epsilon-NTU method"

  extends ThermofluidStream_v2.HeatExchangers.Internal.PartialNTU;
...
equation
  ...
  // For 1 shell pass and even number of tube passes
  effectiveness = if NTU < 1e-6 then NTU else 2 / (1 + C_r + sqrt(1 + C_r^2) * (1 + exp(-gamma)) / (1 - exp(-gamma)));
...
<p>Basic model of a shell and tube heat exchanger (1 shell pass, 2n tube passes) based on the effectiveness-NTU method.</p>
