...
model CurvedBend "Pressure drop due to curved bend using Modelica.Fluid.Dissipation.PressureLoss.Bend"
  ...
  parameter SI.Angle delta "Bending angle (5° - 180°)"
    annotation(Dialog(group="Geometry"));
  parameter SI.Length R "Radius of curvature"
    annotation(Dialog(group="Geometry"));
  parameter ThermofluidStream_v2.Processes.Internal.Material material = ThermofluidStream_v2.Processes.Internal.Material.other "Material of pipe"
    annotation(Dialog(group="Roughness"));
  parameter SI.Length ks "Roughness of pipe"
    annotation(Dialog(group="Roughness"),enable = material == ThermofluidStream_v2.Processes.Internal.Material.other);
  //Initialization
  parameter StateSelect dpStateSelect = StateSelect.default "State select for dp"
    annotation(Dialog(tab = "Advanced"));
  parameter ThermofluidStream_v2.Utilities.Types.InitializationMethods initdp = ThermofluidStream_v2.Utilities.Types.InitializationMethods.none "Initialization method for dp"
    annotation(Dialog(tab = "Initialization", group = "dp"),choicesAllMatching = true);
  parameter SI.Pressure dp_0 = 0 "Initial value for dp"
    annotation(Dialog(tab = "Initialization", group = "Pressure difference", enable = (initdp == ThermofluidStream_v2.Utilities.Types.InitializationMethods.state)));
  parameter ThermofluidStream_v2.Utilities.Units.MassFlowAcceleration dp_acceleraton_0 = 0 "Initial value for der(dp)"
    annotation(Dialog(tab = "Initialization", group = "Pressure difference", enable = (initdp == ThermofluidStream_v2.Utilities.Types.InitializationMethods.derivative)));
  // no default value to require the modeler to think about it; use final to suppress this option to user
  parameter ThermofluidStream_v2.Utilities.Units.Inertance L_value = dropOfCommons.L "Inertance"
...
protected
  ...
  final parameter SI.Length ks_internal=
    if material == ThermofluidStream_v2.Processes.Internal.Material.concrete then 5e-3
    elseif material == ThermofluidStream_v2.Processes.Internal.Material.wood then 0.5e-3
    elseif material == ThermofluidStream_v2.Processes.Internal.Material.castIron then 0.25e-3
    elseif material == ThermofluidStream_v2.Processes.Internal.Material.galvanizedIron then 0.15e-3
    elseif material == ThermofluidStream_v2.Processes.Internal.Material.steel then 0.059e-3
    elseif material == ThermofluidStream_v2.Processes.Internal.Material.drawnPipe then 0.0015e-3
...
equation
  ...
  Xi_out = Xi_in;
  annotation (
    Dialog(enable = (material == ThermofluidStream_v2.Processes.Internal.Material.other)),
    choices(choice = ThermofluidStream_v2.Processes.Internal.Material.concrete "Concrete ks=5mm", choice = ThermofluidStream_v2.Processes.Internal.Material.wood "Wood ks=0.5mm", choice = ThermofluidStream_v2.Processes.Internal.Material.castIron "Cast Iron ks=0.25mm", choice = ThermofluidStream_v2.Processes.Internal.Material.galvanizedIron "Galvanized Iron ks=0.15mm", choice = ThermofluidStream_v2.Processes.Internal.Material.steel "Steel ks=0.059mm", choice = ThermofluidStream_v2.Processes.Internal.Material.drawnPipe "Drawn Pipe ks=0.0015mm"),
...
<p> 
The pipe bend component is using the partial model <a href=\"modelica://ThermofluidStream_v2.Processes.Pipes.Interfaces.SISOFlowBend\">SISOFlowBend</a> implementing the common flow balances.  
...
<p>
<img src=\"modelica://ThermofluidStream_v2/Resources/Doku/Fluid_HTWG/HTWG_en_Markenzeichen_klein_pos_1C.png\" alt=\"HTWG Konstanz\" width=\"350\" height=\"100\">
</p>
</html>"),
