...
partial model partialJunctionY "Duplicate of JunctionN considering dynamic effects"
  ...
  parameter ThermofluidStream_v2.Utilities.Units.Inertance L = dropOfCommons.L "Inertance on each Branch of Component"
    annotation(Dialog(tab = "Advanced"));

  ThermofluidStream_v2.Interfaces.Outlet outlet(redeclare package Medium = Medium) "Outlet"
    annotation(Placement(transformation(extent = {{80, -20}, {120, 20}}), iconTransformation(extent = {{80, -20}, {120, 20}})));
  ThermofluidStream_v2.Interfaces.Inlet inlet_straight(redeclare package Medium = Medium) "Inlet A"
  ...
  ThermofluidStream_v2.Interfaces.Inlet inlet_branching(redeclare package Medium = Medium) "Inlet B"
  ...
  function mfk = ThermofluidStream_v2.Utilities.Functions.massFractionK(redeclare package Medium = Medium);
...
equation
  ...
  annotation (
    ...
    Documentation(info="<html>
<p>
Extension of <a href=\"ThermofluidStream_v2.Topology.JunctionN\">JunctionN</a>. The inlets vector is replaced with two independent inlets and Bernuolli's principle 
